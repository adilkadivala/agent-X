...
`bun run build` runs Next's build-time type-check — TypeScript errors fail the build, not just lint.
...
- `bun run build` type-checks the whole project as part of `next build` — don't rely on the dev server alone to catch type errors.

---

## Linting

ESLint flat config (`eslint.config.mjs`) extends `eslint-config-next` (`core-web-vitals` + `typescript`). Run `bun run lint` before considering frontend changes done.
...
3. **Package manager is bun** — use `bun install`/`bun <script>`; `bun.lock` is the lockfile of record.
4. **Type errors block the build** — `bun run build` runs Next's type-check, so don't leave `any`-typed shortcuts assuming the dev server alone will catch problems.
