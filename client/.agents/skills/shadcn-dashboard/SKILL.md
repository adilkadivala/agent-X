...
- `bun run build` runs Next's type-check before the production build (TypeScript 7 CLI); type errors block it.
- `bun run lint` runs ESLint (flat config extending `eslint-config-next`) — fix warnings, don't suppress them.
- Package manager is **bun** (`bun.lock`) — don't introduce npm/pnpm/yarn lockfiles or commands.
