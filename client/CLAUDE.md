...
- Run `bun run lint` (ESLint flat config extending `eslint-config-next`) before considering frontend changes done.
- `bun run build` runs Next's own type-check as part of the build — type errors block it.
...
No Docker/Netlify config in this repo — standard Next.js deployment (`bun run build` + `bun start`, or a platform like Vercel).
